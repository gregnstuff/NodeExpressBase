...
/* eslint-disable */ const $f60945d37f8e594c$export$4c5dd147b21b9176 = (locations)=>{
    ...
    const map = new mapboxgl.Map({
        container: "map",
        style: "mapbox://styles/gregnstuff/cltw9shy901z801p5506rf2s9",
        scrollZoom: false
    });
...
const $70af9284e599e604$export$596d806903d1f59e = async (email, password)=>{
    try {
        const res = await (0, ($parcel$interopDefault($knI9B$axios)))({
            method: "POST",
            url: "http://127.0.0.1:2999/api/v1/users/login",
            data: {
                email: email,
                password: password
            }
...
const $70af9284e599e604$export$a0973bcfe11b05c9 = async ()=>{
    try {
        const res = await (0, ($parcel$interopDefault($knI9B$axios)))({
            method: "GET",
            url: "http://127.0.0.1:2999/api/v1/users/logout"
        });
...
const $936fcc27ffb6bbb1$export$f558026a994b6051 = async (data, type)=>{
    try {
        ...
        if (res.data.status === "success") (0, $3adf927435cf4518$export$de026b00723010c1)("success", `${type.toUpperCase()} Updated.`);
    } catch (err) {
        console.log(err.response);
        (0, $3adf927435cf4518$export$de026b00723010c1)("error", err.response.data.message);
    }
...
const $6710bca62beba915$var$stripe = Stripe("pk_test_51OzesjGN6vblYd5zHZPbKRLVB0uSAbxRbHRveJQiOCqmr7rCTBhE5AaVVllZPL0scJVuzpLmZPNEIOx964J25F5k009fgaGwL3");
const $6710bca62beba915$export$8d5bdbf26681c0c2 = async (tourId)=>{
    try {
        // 1) Get checkout session from API
        const session = await (0, ($parcel$interopDefault($knI9B$axios)))(`http://127.0.0.1:3000/api/v1/bookings/checkout-session/${tourId}`);
...
const $d0f7ce18c37ad6f6$var$logOutBtn = document.querySelector(".nav_el--logout");
